...
/**
 * These classes set up the GUI of the org.atlanmod.risk.Risk program.
 * The main menu, dialog for setting player count, dialog for name/color settings for each 
 * player, the org.atlanmod.risk.Risk game board, and a menu used during a org.atlanmod.risk.Risk game session are included.
 * @author Ted Mader
 * @version Alpha
 * @date 5/02/14
 **/
public class RiskView extends JFrame {	
	...
	protected RiskView()
	{
		setTitle("Java-org.atlanmod.risk.Risk");
...
class BoardView extends JDialog  {
	...
	/**
	 * Constructs the org.atlanmod.risk.Risk game board.
	 **/
	...
	/**
	 * The panel containing the scrollable org.atlanmod.risk.Risk map image.
	 **/
	private JPanel mapPanel() {
	...
	private JPanel actionPanel() {
		...
		model.addPropertyChangeListener(cardsListModel);
		model.addPropertyChangeListener(countryAListModel);
		model.addPropertyChangeListener(countryBListModel);
	...
	private JPanel countryInfoPanel(Board board) {
		...
		for (i = 0; i < board.getMemberCountries("North America").size(); i++) {
			...
			model.addPropertyChangeListener(countryLabel);
		}
