    ...
    public boolean isFortificationPhaseValid(MapIO mapIO, Player playerPlaying) {
        for (Continent continent : mapIO.getMapGraph().getContinents().values()) {
            for (Country country : continent.getListOfCountries()) {
                // Clause de garde : si le pays n'appartient pas au joueur, on passe au suivant
                if (!country.getPlayer().equals(playerPlaying)) continue;

                // Clause de garde : si le pays n'a pas assez d'armées, on passe au suivant
                if (country.getNoOfArmies() <= 1) continue;

                // Vérification des pays adjacents
                for (Country adjCountry : country.getAdjacentCountries()) {
                    if (adjCountry.getPlayer().equals(playerPlaying)) {
                        return true; // fortification disponible
                    }
                }
            }
        }
        return false; // aucun cas trouvé
    }
