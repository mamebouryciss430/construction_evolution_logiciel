...
public class Benevolent extends PlayerBehaviour {
    ...
    public Benevolent(GamePlayController gamePlayController) {
        this.addObserver(gamePlayController); // champ inutile supprimé
    }
	 ...
	 */
        ...
        public void reinforcementPhase(ObservableList<Country> countryList, Country country, Player currentPlayer) {
            ...
            if (!sortedList.isEmpty()) {
                Country weakestCountry = sortedList.get(0); // variable locale au lieu du paramètre
                weakestCountry.setNoOfArmies(weakestCountry.getNoOfArmies() + currentPlayer.getArmyCount());

                System.out.println(currentPlayer.getName() + " benevolent player has been assigned all the "
                        + currentPlayer.getArmyCount() + " armies to the weakest country " + weakestCountry.getName());
                System.out.println("Ended Reinforcement phase for benevolent player " + currentPlayer.getName());

                setChanged();
                notifyObservers(currentPlayer.getName() + " benevolent player has been assigned all the "
                        + currentPlayer.getArmyCount() + " armies to the weakest country " + weakestCountry.getName() + ".\n");
