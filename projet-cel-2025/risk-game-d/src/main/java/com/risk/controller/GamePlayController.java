...
public class GamePlayController implements Initializable, Observer, Externalizable {
    ...
    private void attack() {
        ...
        PlayerPhaseManager.attackPhase(selectedCountryList, adjacentCountryList, gamePlayerList, playerPlaying, playerPlaying);
    }
    ...
    private void fortify(ActionEvent event) {
        PlayerPhaseManager.fortificationPhase(selectedCountryList, adjacentCountryList, gamePlayerList, playerPlaying, playerPlaying);
    ...
    private void placeArmy(ActionEvent event) {
        PlayerPhaseManager.placeArmyOnCountry(selectedCountryList, gamePlayerList, playerPlaying, playerPlaying);
    }
    ...
    private void reinforcement(ActionEvent event) {
        ...
        PlayerPhaseManager.reinforcementPhase(selectedCountryList.getItems(), country, gamePlayerList, playerPlaying, playerPlaying);
        selectedCountryList.refresh();
        loadMapData();
    ...
    private void initializeAttack() {
        ...
        if (PlayerPhaseManager.playerCanAttack(selectedCountryList, playerPlaying, playerPlaying)) {
    ...
    private void isAnyPlayerLost() {
        List<Player> playersLost = PlayerVictoryManager.playersWhoLost(gamePlayerList, playerPlaying, playerPlaying);
    ...
    private void isValidFortificationPhase() {
        PlayerPhaseManager.isFortificationPhaseValid(map, playerPlaying, playerPlaying);
    }
    ...
    public void exchangeCards(Card exch) {
        ...
        CardManager.exchangeCards(playerPlaying, tradedCards, getNumberOfCardSetExchanged(), playerPlaying);
    ...
    public BufferedWriter clearContentsOfFile() {
        ...
        try {
            String tmpdir = Files.createTempDirectory("risk-game-d").toFile().getAbsolutePath();
            String timestamp = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
            String fileName = Paths.get(tmpdir, "risk-game-d", timestamp).toString() + ".txt";
