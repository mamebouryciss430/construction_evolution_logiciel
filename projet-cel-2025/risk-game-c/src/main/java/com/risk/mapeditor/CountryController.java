    ...
    public void initialize(Country country){
        ...
        btnRemove.setOnMouseClicked(event ->  {
            ...
            for (Edge line:edgeList){
                ...
                p1.removeEdge(line);
                p2.removeEdge(line);
                AnchorPane drawPane=(AnchorPane) line.getParent();
                drawPane.getChildren().remove(line);
            }
            AnchorPane drawPane=(AnchorPane) country.getParent();
            View.getViewController().removeFromCountryList(country);
            drawPane.getChildren().remove(country);
            event.consume();
        });
