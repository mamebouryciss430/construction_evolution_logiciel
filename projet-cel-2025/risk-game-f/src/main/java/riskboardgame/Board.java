...
public class Board extends TelegramLongPollingBot{
	...
	public Board(HashMap<String, Country> m, Deck d)	{
		System.out.println("=^-^= riskboardgame.Risk riskboardgame.Board Game");
		this.numOfPlayers = 0;
		this.gameID = "";
		this.openGameNum = 0;
	...
	public String showAllCommands()
	{
		String tempStr = "Lists of Commands\n"
				+ "=creategame [number of riskboardgame.Player] [gameid]\n"
				+ "=showgameid\n"
				+ "=join [gameid]\n"
				+ "=determineturns\n"
				+ "=showturns"
    ...
    private int[] resolveBattle(int[] attackerRolls, int[] defenderRolls) {
        int attackerLose = 0;
        int defenderLose = 0;

        int battles = Math.min(attackerRolls.length, defenderRolls.length);

        for (int i = 0; i < battles; i++) {
            if (attackerRolls[attackerRolls.length - 1 - i] >
                    defenderRolls[defenderRolls.length - 1 - i]) {
                defenderLose++;
            } else {
                attackerLose++;
            }
        ...
        return new int[]{attackerLose, defenderLose};
    }
    public String attack(int attackerIndex, String aCountry, String dCountry) {
        ...
        int[] attackerRolls = dice.roll(
                dice.maxNumDice("Attacker", attackerCountry.getNumOfArmy()));
        int[] defenderRolls = dice.roll(
                dice.maxNumDice("Defender", defenderCountry.getNumOfArmy()));

        int[] losses = resolveBattle(attackerRolls, defenderRolls);
        int attackerLose = losses[0];
        int defenderLose = losses[1];

        Map.get(aCountry).loseNumOfArmy(attackerLose);
        Map.get(dCountry).loseNumOfArmy(defenderLose);

        if (Map.get(dCountry).getNumOfArmy() < 1) {
            transferOwnership(aCountry, attackerIndex, dCountry, defenderIndex);

            if (players.get(defenderIndex).getOwnedCountries().isEmpty()) {
                killPlayer(defenderIndex);
            }
        }

        return players.get(attackerIndex).getName() + " Lost : " + attackerLose +
                "\n" + players.get(defenderIndex).getName() + " Lost : " + defenderLose;
    }
    ...
    public String showCountryValidation(String name, String country, int dir) {
        ...
        int status = checkAttackableOrFortifiable(name, country);

        if (status != 1) {
            switch (status) {
                case -1:
                    return "Invalid riskboardgame.Country Name";
                case -2:
        ...
        Country baseCountry = Map.get(country);
        ArrayList<String> connections = baseCountry.getAdjacency();

        if (connections.isEmpty()) {
            return "There is no country you can attack";
        }

        StringBuilder result = new StringBuilder("Here is/are the lists\n");

        for (String adj : connections) {
            Country c = Map.get(adj);

            if (shouldDisplayCountry(c.getOwnerName(), name, dir)) {
                result.append(c.getCountryName())
                        .append(" : ")
                        .append(c.getNumOfArmy())
                        .append("\n");
            }
        }
    ...
    private boolean shouldDisplayCountry(String owner, String player, int dir) {
        return (dir == 1 && !owner.equals(player)) ||
                (dir == 2 && owner.equals(player));
    }
	...
	public String armyPlacement()
	{
		...
		while (players.get(this.numOfPlayers - 1).getNumOfTroops()> 0)
		{
			if (!firstRound)
			{
				...
				Map.get(pickedCountry).setOwnerName(players.get(playerIndex).getName());
			...
			{
				pickedCountry = this.mapKeySets.get(i);
				if (Map.get(pickedCountry).getOwnerName().equals(players.get(playerIndex).getName()))
				{
					Map.get(pickedCountry).addNumOfArmy(1);
					players.get(playerIndex).loseNumOfTroops(1);
					playerIndex++;
	...
	{
		System.out.format(format, "Index", "Region" ,"riskboardgame.Country Name", "Land Owner", "Num of Army");
		int i = 0;
	...
	public String checkCategory(String userName, long user_id, String txt)
	{
		...
		} else if (txt.contains("=join"))
		{
			...
			{		
				...
				{
					this.gameStart = true;
					botResponseToAll("Game Start\n" + "The Current Turn : " + players.get(0).getName());
		...
		} else if (txt.equals("=myinfo"))
		{
			...
			{
				if (players.get(i).getName().equals(userName))
					return "My Name : " + players.get(i).getName() + "\nMy Number of Troops : "
		...
		} else if (txt.contains("=attack"))
		{
			...
			if (currentPlayerIndex != getPlayerIndex(userName))
				return "Not your turn. Current Turn : " + players.get(currentPlayerIndex).getName();
		...
		} else if (txt.contains("=showattackable"))
		{
			...
			if (this.currentActionIndex == 2)
			{
				this.currentActionIndex = 0;
				this.lastAction = "";
				String cpname = players.get(currentPlayerIndex).getName();
				String npname = (currentPlayerIndex == this.numOfPlayers - 1) ? players.get(0).getName() : players.get(currentPlayerIndex + 1).getName();
		...
		} else if (txt.equals("=showturns"))
		{
			...
			for (int i = 0; i < this.numOfPlayers; i++)
			{
				tempStr += players.get(i).getName() + " > ";
			}
			tempStr += "\nCurrent riskboardgame.Player : " + players.get(this.currentPlayerIndex).getName();
